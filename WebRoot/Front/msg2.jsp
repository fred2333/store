...
a:hover {color:#FF6600; text-decoration: underline;}
  ...
  <c:choose>
  	<c:when test="${code eq 'success' }"><%--如果code是功能，它显示对号图片 --%>
  		<c:set var="img" value="/Front/images/duihao.jpg"/>
  		<c:set var="title" value="成功"/>
  	</c:when>
  	<c:when test="${code eq 'error' }"><%--如果code是功能，它显示错号图片 --%>
  		<c:set var="img" value="/Front/images/cuohao.png"/>
...
<div class="divBody">
	...
	<div class="divContent">
	  <div style="margin: 20px;">
		...
		<span style="margin-left: 50px;"><a target="_top" href="<c:url value='/OrderServlet?method=cancel2&oid=${oid}'/>">确认</a></span>
		</c:if>
