package cn.edu.zhk.jsj141.feng.ctrl.user;
...
import cn.edu.zhk.jsj141.feng.service.user.UserService;
...
public class UserServlet extends BaseServlet {
	...
	public String regist(HttpServletRequest req, HttpServletResponse resp)
		...
		if(errors.size() > 0) {
			...
			return "f:/Front/user/regist.jsp";
		}
		/*
		 * 3. 使用service完成业务
		 */
	...
	public String updatePassword(HttpServletRequest req, HttpServletResponse resp)
		...
		if(user == null) {
			...
			return "f:/Front/user/login.jsp";
		}
		
		try {
			String loginpass = formUser.getLoginpass();
			String newpass = formUser.getNewpass();
			String 	repass = formUser.getReloginpass();
	 ...
	 * @throws IOException
	 */
	public String updateU(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		/*
		 * 1. 封装表单数据到user中
		 * 2. 从session中获取uid
		...
		try {
			String address = formUser.getAddress();
			String email = formUser.getEmail();
			String phone = formUser.getPhone();
			if(email == null || email.trim().isEmpty()) {
				req.setAttribute("msg", "邮箱不能为空！");
				return "f:/Front/user/edit.jsp";
			}
			else if(!email.matches("^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\\.[a-zA-Z0-9_-]{2,3}){1,2})$")){
				req.setAttribute("msg", "邮箱格式错误！");
				return "f:/Front/user/edit.jsp";
			}
			else if(address == null || address.trim().isEmpty()){
				req.setAttribute("msg", "地址不能为空！");
				return "f:/Front/user/edit.jsp";
			}
			else if(address.length()>64){
				req.setAttribute("msg", "地址长度不能超出64位！");
				return "f:/Front/user/edit.jsp";
			}
			else if(phone == null || phone.trim().isEmpty()){
				req.setAttribute("msg", "手机号码不能为空！");
				return "f:/Front/user/edit.jsp";
			}
			else if(phone.length()!=11){
				req.setAttribute("msg", "手机号码必须是11位的！");
				return "f:/Front/user/edit.jsp";
			}
			userService.updateU(user.getUid(), formUser.getEmail(), 
					formUser.getPhone(),formUser.getAddress());
